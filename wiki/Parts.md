...
> 两项元件均提供 **FASTA、GenBank、metadata、characterization 和 map**。最终同步检查确认 FASTA 与 GenBank 序列一致，元件编号、创建年份、原 Registry 来源和许可信息均保留。
...
| 元件 | FASTA | GenBank | Metadata | Characterization | Map | 序列一致性 |
|---|:---:|:---:|:---:|:---:|:---:|:---:|
| **AISB26-045-001** | ✓ | ✓ | ✓ | ✓ | ✓ | **FASTA = GenBank** |
| **AISB26-045-002** | ✓ | ✓ | ✓ | ✓ | ✓ | **FASTA = GenBank；CDS = 1..690** |

完整索引：[`parts/README.md`](../parts/README.md) · 完整性审核：[Verifiability](./Verifiability.md#4-元件文件完整性)
...
| 元件 | 对应模块 | 项目作用 |
|---|---|---|
| **AISB26-045-001 · PhiReX** | P2 Color & Light Translator | 为光控表达、光输入参数和硬件接口提供实验基础 |
| **AISB26-045-002 · BmCBP** | P1 Material Interface Optimizer | 为色素结合、织物实验和固色材料选择提供蛋白元件基础 |
